...
void show_statistics(sf::RenderWindow &window, class VerletSolver &solver, sf::Font &font, double dt);
void spawn_objects(class VerletSolver &solver, double dt);

sf::Font font;
bool spawn = true;

std::vector<unsigned> fps_choices = {30, 60, 75, 100, 144, 240, 360, 750, 1000};
unsigned fps_choice = (unsigned)fps_choices.size() - 1u;
...
void User::OnUpdate(sf::RenderWindow &window, double dt)
{
    ...
    if (spawn_clock.getElapsedTime().asSeconds() > 0.001f && sf::Mouse::isButtonPressed(sf::Mouse::Left))
    {
        ...
        solver.add_object(position, radius, getRainbow(total_clock.getElapsedTime().asSeconds()));
    }
    ...
    {
        sf::CircleShape shape((float)object.radius);
...
void spawn_objects(class VerletSolver &solver, double dt)
{
    ...
    if (spawn_clock.getElapsedTime().asSeconds() > 0.500f && spawn)
    {
        spawn_clock.restart();
